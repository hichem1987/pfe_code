...
class ONE_RevSlider_SliderController extends Mage_Adminhtml_Controller_Action{
    public function _initAction(){
        $this->loadLayout()->_setActiveMenu('one/revslider');
        $this->_title(Mage::helper('revslider')->__('ONE Extensions'));
        return $this;
    }
    ...
    public function editAction(){
        ...
        $this->_initAction();
        Mage::helper('oneext')->loadJsLibs(array('jscolor', 'browser', 'codemirror'));
        $this->_title(Mage::helper('revslider')->__('Manage Slider'));
    ...
    public function saveAction(){
        if ($data = $this->getRequest()->getPost()){
            ...
            try{
                $model->save();
                Mage::app()->getCache()->clean('matchingTag', array(ONE_RevSlider_Model_Slider::CACHE_TAGS));
    ...
    public function addSlideAction(){
        ...
        $this->_initAction();
        Mage::helper('oneext')->loadJsLibs(array('jscolor', 'codemirror', 'browser'));
        $this->_title(Mage::helper('revslider')->__('Manage Slider'));
    ...
    public function importAction(){
        $this->_initAction();
        $this->_setActiveMenu('one/revslider');
        $this->_title(Mage::helper('revslider')->__('Import Revolution Slider'));
        $this->renderLayout();
    }
        ...
        if ($this->getRequest()->isPost()){
            try{
                ...
                if ($slider->getId()) {
                    ...
                    /* @var $helper ONE_RevSlider_Helper_Data */
    ...
    protected function _processZipImport($filePath){
        ...
        if ($sliderData !== false){
            ...
            /* @var $cssParser ONE_RevSlider_Helper_Css */
            ...
            if (is_array($cssArray)){
                foreach ($cssArray as $cssRule => $cssProperties){
                    ...
                    /* @var $cssModel ONE_RevSlider_Model_Css */
        ...
        if ($animationData !== false){
            ...
            if ($animationArray !== false && is_array($animationArray)){
                foreach ($animationArray as $animation){
                    ...
                    /* @var $animationModel ONE_RevSlider_Model_Animation */
