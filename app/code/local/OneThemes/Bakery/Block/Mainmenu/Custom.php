...
class OneThemes_Bakery_Block_Mainmenu_Menu_Custom extends Mage_Catalog_Block_Navigation 
{
     protected $_column;
    protected $_isMomenu;
    protected $_isSmart;
                                                   ...
                                                   $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if(!$catdetail->getData('onemenu_is_category') && $level==0){
            return '';
        }
        $html = array();
        ...
        $menutypes = $catdetail->getData('onemenu_cat_groups');

        // Check if show category block if no sub-category
        $showblock = false;
        $showblock = $hasActiveChildren;
        if (Mage::helper('bakery')->getCfg('menu/show_if_no_children')) {$showblock = true; }
        ...
        if ($level == 1 && $showblock){
            if($this->_getCatBlock($catdetail, 'onemenu_cat_block_top')){
                $html[] = '<div class="onemenu-block onemenu-block-level1-top std">';
                $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_top');
                $html[] = '</div>';
            }
        }
        ...
        if ($level == 0) {
            $cat_block_right = $this->_getCatBlock($catdetail, 'onemenu_cat_block_right');
            $cat_block_left = $this->_getCatBlock($catdetail, 'onemenu_cat_block_left');
            if ($catdetail->getData('onemenu_proportions_right') || $catdetail->getData('onemenu_proportions_left')) {
                $columns = $catdetail->getData('onemenu_cat_columns');
                $proportion_right = $catdetail->getData('onemenu_proportions_right');
                $proportion_left = $catdetail->getData('onemenu_proportions_left');
            } else {
                if($catdetail->getData('onemenu_cat_columns')==''){
                    $columns = 2;
                }else{
                    $columns = $catdetail->getData('onemenu_cat_columns');
                }
        ...
        if ($childrenWrapClass && $showblock) { 
            if($menutypes == 'drop_group'){
                if($level==1){
                    ...
                    $html[] = '<div class="mm-item-base clearfix">';
                }
        ...
        if($level==0 && $showblock){
            if($this->_getCatBlock($catdetail, 'onemenu_cat_block_top')){
                $html[] = '<div class="onemenu-block onemenu-block-top grid-full std">';
                $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_top');
                $html[] = '</div>';
            }
            if($menutypes != 'drop_group'){
                if($this->_getCatBlock($catdetail, 'onemenu_cat_block_left') && $proportion_left){
                    $html[] = '<div class="menu-static-blocks onemenu-block onemenu-block-left '.$gridCountLeft.'">';
                    $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_left');
                    $html[] = '</div>';
                }
            }
        }
        if (!empty($li) && $hasActiveChildren) {
            if($level==0){
                ...
                $html[] = '<div class="onemenu-block onemenu-block-center menu-items '.$gridCount1.' '. $colCenter .'">';
            }
        ...
        if($level==0 && $showblock){
            if($menutypes != 'drop_group'){
                if($this->_getCatBlock($catdetail, 'onemenu_cat_block_right') && $proportion_right){
                    $html[] = '<div class="menu-static-blocks onemenu-block onemenu-block-right '.$gridCountRight.'">';
                    $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_right');
                    $html[] = '</div>';
                }
            }
            if($this->_getCatBlock($catdetail, 'onemenu_cat_block_bottom')){
                $html[] = '<div class="onemenu-block onemenu-block-bottom grid-full std">';
                $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_bottom');
                $html[] = '</div>';
            }
        }
        if ($childrenWrapClass && $showblock) {
                                                   ...
                                                   $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        $catdetail = $this->_getCatInfo($category->getId());
        if(!$catdetail->getData('onemenu_is_category') && $this->_isMomenu == FALSE){
            return '';
        }
        $html = array();
        ...
        if ($level == 1 && $showblock && $this->_isMomenu == FALSE){
            if($this->_getCatBlock($catdetail, 'onemenu_cat_block_bottom') && $menutypes != 'dropdown'){
                $html[] = '<div class="onemenu-block onemenu-block-level1-top std">';
                $html[] = $this->_getCatBlock($catdetail, 'onemenu_cat_block_bottom');
                $html[] = '</div>';
            }
    ...
    public function renderCategoriesMenuHtml($momenu = FALSE, $smart = TRUE, $level = 0, $outermostItemClass = '', $childrenWrapClass = '')
    {
        ...
        foreach ($activeCategories as $category) {
            if($this->_isMomenu){
                ...
                $menutype = $catdetail->getData('onemenu_cat_groups');
    ...
    protected function _getCategoryLabelHtml($category, $level){
        $labelCategory = $category->getData('onemenu_cat_label');
        if ($labelCategory){
            $getLabel = trim(Mage::helper('bakery')->getCfg('menu/' . $labelCategory));
            if ($getLabel) {
                if ($level == 0){
