 ...
 */
class OneThemes_Bakery_Model_Observer
{
	/**
	...
	public function configSave()
	{
        ...
        Mage::log($section);
		if ($section == 'bakery_design')
		{
			$websiteCode = Mage::app()->getRequest()->getParam('website');
			$storeCode = Mage::app()->getRequest()->getParam('store');
			
			Mage::getSingleton('bakery/cssgen_generator')->generateCss('design', $websiteCode, $storeCode);
		}else if($section == 'bakery'){
            $websiteCode = Mage::app()->getRequest()->getParam('website');
            $storeCode = Mage::app()->getRequest()->getParam('store');

            Mage::getSingleton('bakery/cssgen_generator')->generateCss('layout', $websiteCode, $storeCode);
        }
    ...
    public function catalogBlockProductCollectionBeforeToHtml(Varien_Event_Observer $observer)
    {
        ...
        if ($productCollection instanceof Varien_Data_Collection) {
            ...
            Mage::getModel('bakery/collection')->appendType($productCollection);
        }
