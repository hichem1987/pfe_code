...
abstract class OneThemes_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = OneThemes_Blog_Helper_Data::CATEGORY_URI_PARAM;

    protected static $_postUriParam = OneThemes_Blog_Helper_Data::POST_URI_PARAM;

    protected static $_tagUriParam = OneThemes_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new OneThemes_Blog_Helper_Substring(
                    array(
    ...
    {
        if (self::$_helper->conf(OneThemes_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('onethemes_blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('onethemes_blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {
        return '<a class="onethemes-blog-read-more" href="' . $item->getAddress() . '">' . $this->__('Read More') . '</a>';
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == OneThemes_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $collection = Mage::getModel('blog/blog')->getCollection()
                ->addPresentFilter()
                ->addEnableFilter(OneThemes_Blog_Model_Status::STATUS_ENABLED)
                ->addStoreFilter()
                ->joinComments()
