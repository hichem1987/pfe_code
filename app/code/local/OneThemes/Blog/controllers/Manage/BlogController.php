...
class OneThemes_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action
{
        ...
        if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['thumbnailimage']['name']) && $_FILES['thumbnailimage']['name'] != ''){
                try{
                    ...
                    $path = Mage::getBaseDir('media').DS.'onethemes/blog/'.DS;
                    $result = $uploader->save($path,$_FILES['thumbnailimage']['name']);
                    
                    //this way the name is saved in DB
                    $data['thumbnailimage'] = 'onethemes/blog/'.$result['file'];
                }catch(Exception $e){
