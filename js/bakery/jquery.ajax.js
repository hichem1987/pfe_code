...
function showBox(message,wait){
    $one('#notification').html('<div class="success" style="display: none;">' +
        '<i class="fa fa-check"></i>' +
        '' + message + '' +
        '<button class="close"><i class="fa fa-times-circle"></i></button></div>');
    $one('.success').fadeIn('slow');
    if(wait){
        $one('.success').addClass('wait-loading');
        $one('.success .fa-check').hide();
        $one('.success .close').hide();
    }else{
        $one('.success').removeClass('wait-loading');
        $one('.success .fa-check').show();
        $one('.success .close').show();
    }
    $one('.success').fadeIn('slow');
    if(!wait){
        setTimeout(function() {
            $one('.success').delay(500).fadeOut(1000);
        }, 4000);
    }
}
